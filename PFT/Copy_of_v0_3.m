...
W_mirror = zeros(length(theta), length(alpha_jouk));
W_combined = zeros(length(theta), length(alpha_jouk));
...
litteral_complex_pot = @(gamma, alpha) (U_inf.*exp(-1i.*alpha) + 1i.*gamma./(2*pi.*(zeta_circ-zeta_0)) - U_inf.*R_joukowski^2.*exp(1i.*alpha)./((zeta_circ-zeta_0).^2))./(1-a^2./(zeta_circ.^2));

% Coefficients computation
for i = 1:length(alpha_jouk)
    W_joukowski(:, i) = litteral_complex_pot(Gamma_joukowski(i), alpha_jouk(i));
 
    % Mirror-image potential for ground effect
    W_mirror(:, i) = conj(litteral_complex_pot(-Gamma_joukowski(i), alpha_jouk(i)));

    % Combined potential (original + mirror-image)
    W_combined(:, i) = W_joukowski(:, i) + W_mirror(:, i);

    u_joukowski(:, i) = real(W_combined(:, i));
    v_joukowski(:, i) = -imag(W_combined(:, i));
...
fig_cp_combined = figure;
plot(x_a5, Cp_a5_jouk, '-ob')
xlim([0, 1])
xlabel('x/c')
ylabel('C_p')
legend('Cp_{combined}', 'Location', 'southeast')
