...
z_plane_joukowsi = zeta_plane + a^2./zeta_plane;
...
for i = 1:length(alpha_jouk)
    ...
    W_pot_tot(:, :, i) = W_pot_plane(:, :, i) + W_pot_plane_mir(650:1049, :, i);
...
contour(real(zeta_plane), imag(zeta_plane), v_pot_plane(:, :, index_a5_jouk), 100)
grid on;
hold on;
...
contour(real(zeta_plane_mir), imag(zeta_plane_mir), v_pot_plane_mir(:, :, index_a5_jouk), 400)
grid on;
hold on;
% ylim([-2, 2]);
% xlim([-2, 2]);
...
contour(real(z_plane_joukowsi), imag(z_plane_joukowsi), v_pot_tot(:, :, index_a5_jouk), 100)
grid on;
hold on;
%fill(real(z_wing_joukowski), imag(z_wing_joukowski), 'y')
