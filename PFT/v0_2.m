...
h_ground = 0.1;  % Distance from airfoil to the ground
...
c_n_ba = zeros(1, length(alpha_jouk));
cl_ba = zeros(1, length(alpha_jouk));
cl_kj = zeros(1, length(alpha_jouk));
...
for i = 1:length(alpha_jouk)
    % Original potential flow term
    W_joukowski(:, i) = (U_inf.*exp(-1i.*alpha_jouk(i)) + ...
        1i.*Gamma_joukowski(i)./(2*pi.*(zeta_circ-zeta_0)) - ...
        U_inf.*R_joukowski^2.*exp(1i.*alpha_jouk(i))./((zeta_circ-zeta_0).^2))./(1-a^2./(zeta_circ.^2)) ...
        + (U_inf.*exp(-1i.*alpha_jouk(i)) + ...
        1i.*Gamma_joukowski(i)./(2*pi.*(conj(zeta_circ)-conj(zeta_0))) - ...
        U_inf.*R_joukowski^2.*exp(1i.*alpha_jouk(i))./((conj(zeta_circ)-conj(zeta_0)).^2))./(1-a^2./(conj(zeta_circ).^2));

    % % Virtual image term (reflected below the ground)
    % W_joukowski(:, i) = W_joukowski(:, i) + (U_inf.*exp(-1i.*alpha_jouk(i)) + ...
    %     1i.*Gamma_joukowski(i)./(2*pi.*(conj(zeta_circ-zeta_0))) - ...
    %     U_inf.*R_joukowski^2.*exp(1i.*alpha_jouk(i))./((conj(zeta_circ-zeta_0)).^2))./(1-a^2./(conj(zeta_circ).^2));
